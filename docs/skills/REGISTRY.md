...
| `0web-experience-design-max` | `.agents/skills/0web-experience-design-max` | APPROVED_GLOBAL | experience design / UI UX / motion | toda UI material, landing, portfolio, redesign | autoral 0WEB com princípios revisados de fontes externas | interna | sem scripts externos |
...
|---|---|---|
| `nextlevelbuilder/ui-ux-pro-max-skill@7f69fed6a2717900085f1bc3b263721f8ba025e2` | APPROVED_GLOBAL (princípios/design intelligence) | MIT. O `SKILL.md` é forte em a11y, touch, performance, layout, tipografia, motion, forms e stack guidance. O repositório amplo contém scripts/subprocessos e integrações auxiliares; portanto a distribuição completa não é executada automaticamente. Os princípios revisados foram internalizados em `0web-experience-design-max`. |
| Dexa Experience Design | APPROVED_GLOBAL (processo/repertório) | Adotado como referência de processo integrado: estratégia digital, UX/UI, websites, produto, design system, motion e Design Ops. Não copiar layout, marca ou assets da agência. |
| LobeHub Skills Marketplace / `@lobehub/market-cli` | APPROVED_GLOBAL (discovery) | Fonte obrigatória de descoberta complementar quando a tarefa substancial pedir skill ausente. Instalação somente após revisão de `SKILL.md`, scripts, dependências, licença e fonte original. Nunca no build/deploy. |
| AwesomeSkill | APPROVED_GLOBAL (discovery) | Útil para localizar/comparar skills e sinalizar riscos, mas não substitui fonte original nem revisão local. A página de UI/UX Pro Max sinaliza risco médio por instruções de privilégio; por isso o 0WEB não executa install/OS mutation automaticamente. |
...
| `lobehub-skills-search-engine` | SECURITY_REVIEW_REQUIRED | O identificador solicitado é reconhecido como candidato de discovery. Antes de instalar/usar código, localizar a fonte original e revisar arquivos. O 0WEB já incorpora LobeHub como fonte de descoberta sem depender desse pacote para build/runtime. |
...
direção criativa + CRO + UX/a11y + React/performance + motion + SEO/conteúdo +
privacidade/segurança + QA. Skills que repetem o mesmo `hero + features + testimonials + FAQ`
são redundantes e devem ser rejeitadas.

## Regra de instalação

Nenhuma nova dependência externa deve ser instalada só para satisfazer uma skill.
Instalação futura exige `SECURITY.md`, revisão de scripts/dependências, fonte original
e commit SHA. Princípios seguros podem ser incorporados às skills autorais sem executar
código de terceiros.
