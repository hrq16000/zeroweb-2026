...
const CAPABILITIES = ["Marca", "Sites", "Presença local", "SEO", "Tráfego", "Automação", "IA"] as const;
...
export function Home2PersonalityExperience() {
  ...
  useEffect(() => {
    ...
    const observedSections = Array.from(shell.querySelectorAll<HTMLElement>("#sobre, #solucoes, #processo, #projetos, #conteudo"));
    ...
    if (typeof IntersectionObserver !== "undefined" && observedSections.length) {
      sectionObserver = new IntersectionObserver(
        (entries) => {
          ...
          if (visible?.target instanceof HTMLElement) shell.dataset.activeSection = visible.target.id;
        },
  ...
  return (
    <div className="home2-personality-shell" ref={shellRef}>
      <div className="home2-page-progress" aria-hidden="true"><span /></div>
      ...
      {HERO_PROJECTS.length > 0 ? (
        <div className="home2-prime-hero-showcase" aria-hidden="true">
          ...
          {HERO_PROJECTS.map((project, index) => (
            <span key={project.slug} className={`home2-prime-project home2-prime-project--${String.fromCharCode(97 + index)}`}>
              <PortfolioCover
      ...
      <div className="home2-scroll-cue" aria-hidden="true"><span>Explore</span><i /></div>
      <Home2Prototype />
    </div>
  );
}
